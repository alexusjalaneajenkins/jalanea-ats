# Theme and Design Tokens

Theme summary:
- Tailwind CSS v4 is loaded via `@import "tailwindcss";` in `src/app/globals.css`
- No standalone `tailwind.config.ts` or `tailwind.config.js` file is present
- Global fonts are a mix of CSS custom properties (`DM Sans`, `Space Grotesk`, `Fira Code`) and `next/font` Geist variables in `src/app/layout.tsx`
- Visual direction in the current app is dark, neon, gradient-heavy, and indigo/orange/pink-forward

## Root Layout Font Setup
- File: `src/app/layout.tsx`

```tsx
import type { Metadata, Viewport } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import { PWAInstall } from "@/components/PWAInstall";
import "./globals.css";

const geistSans = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
});

const geistMono = Geist_Mono({
  variable: "--font-geist-mono",
  subsets: ["latin"],
});

export const metadata: Metadata = {
  title: "Jalanea ATS - Resume Checker",
  description: "Check your resume's ATS compatibility score before applying. Get keyword analysis, formatting feedback, and actionable suggestions to optimize your resume.",
  keywords: ["ATS", "resume", "job search", "applicant tracking system", "resume optimizer", "career"],
  authors: [{ name: "Jalanea" }],
  manifest: "/manifest.json",
  themeColor: "#f97316",
  appleWebApp: {
    capable: true,
    statusBarStyle: "black-translucent",
    title: "Jalanea ATS",
  },
  openGraph: {
    title: "Jalanea ATS - Resume Checker",
    description: "Check your resume's ATS compatibility score before applying.",
    type: "website",
  },
  icons: {
    icon: [
      { url: "/icons/icon.svg", type: "image/svg+xml" },
    ],
    apple: [
      { url: "/icons/icon-192.png", sizes: "192x192" },
    ],
  },
};

export const viewport: Viewport = {
  width: "device-width",
  initialScale: 1,
  maximumScale: 5,
  themeColor: "#f97316",
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body
        className={`${geistSans.variable} ${geistMono.variable} antialiased bg-forge-950`}
      >
        {children}
        <PWAInstall />
      </body>
    </html>
  );
}
```

## Global CSS
- File: `src/app/globals.css`

```css
@import "tailwindcss";

/* Jalanea ATS - Nighttime Mischief Design System */
:root {
  --font-sans: "DM Sans", system-ui, sans-serif;
  --font-display: "Space Grotesk", system-ui, sans-serif;
  --font-mono: "Fira Code", monospace;
}

/* Base styles */
html {
  -webkit-text-size-adjust: 100%;
  scroll-behavior: smooth;
  height: 100%;
  background-color: #121218;
}

body {
  /* Dark mode optimized: #121212 base (not pure black) for reduced eye strain */
  background-color: #121218;
  background-image: linear-gradient(135deg, #121218 0%, #1a1333 25%, #0f172a 50%, #1e1b4b 75%, #121218 100%);
  background-repeat: no-repeat;
  background-size: 100% 100%;
  background-attachment: fixed;
  /* Text: #E0E0E0 equivalent (not pure white) for accessibility */
  color: #e0e7ff;
  font-family: var(--font-sans);
  min-height: 100%;
}

/* Selection color */
::selection {
  background: rgba(236, 72, 153, 0.3);
  color: #fce7f3;
}

/* Custom Scrollbar */
::-webkit-scrollbar {
  width: 8px;
  height: 8px;
}

::-webkit-scrollbar-track {
  background: rgba(30, 27, 75, 0.5);
}

::-webkit-scrollbar-thumb {
  background: rgba(99, 102, 241, 0.5);
  border-radius: 4px;
}

::-webkit-scrollbar-thumb:hover {
  background: rgba(99, 102, 241, 0.7);
}

/* Animations */
@keyframes pulse-glow {
  0%, 100% {
    box-shadow: 0 0 20px rgba(249, 115, 22, 0.4), 0 0 40px rgba(236, 72, 153, 0.2);
  }
  50% {
    box-shadow: 0 0 40px rgba(249, 115, 22, 0.6), 0 0 60px rgba(236, 72, 153, 0.3);
  }
}

@keyframes gradient-shift {
  0%, 100% { background-position: 0% 50%; }
  50% { background-position: 100% 50%; }
}

/* Celebration animation for high scores (>85) */
@keyframes celebrate {
  0% { transform: scale(1); }
  15% { transform: scale(1.08); }
  30% { transform: scale(1); }
  45% { transform: scale(1.04); }
  60% { transform: scale(1); }
  100% { transform: scale(1); }
}

/* Attention pulse for low scores (<50) - draws user attention */
@keyframes attention-pulse {
  0%, 100% {
    box-shadow: 0 0 0 0 rgba(239, 68, 68, 0.4);
  }
  50% {
    box-shadow: 0 0 0 8px rgba(239, 68, 68, 0);
  }
}

/* Subtle glow animation for score reveal */
@keyframes score-reveal {
  0% {
    opacity: 0;
    transform: scale(0.9);
  }
  100% {
    opacity: 1;
    transform: scale(1);
  }
}

/* Floating animation for clouds and badges */
@keyframes float {
  0%, 100% {
    transform: translateY(0px);
  }
  50% {
    transform: translateY(-8px);
  }
}

@keyframes float-slow {
  0%, 100% {
    transform: translateY(0px) rotate(var(--rotate, 0deg));
  }
  50% {
    transform: translateY(-12px) rotate(var(--rotate, 0deg));
  }
}

/* Twinkling animation for stars */
@keyframes twinkle {
  0%, 100% {
    opacity: 0.3;
    transform: scale(0.8);
  }
  50% {
    opacity: 1;
    transform: scale(1.2);
  }
}

@keyframes twinkle-slow {
  0%, 100% {
    opacity: 0.4;
    transform: scale(1);
  }
  50% {
    opacity: 1;
    transform: scale(1.1);
  }
}

/* Animation utilities */
.animate-float {
  animation: float 4s ease-in-out infinite;
}

.animate-float-slow {
  animation: float-slow 6s ease-in-out infinite;
}

.animate-twinkle {
  animation: twinkle 2s ease-in-out infinite;
}

.animate-twinkle-slow {
  animation: twinkle-slow 3s ease-in-out infinite;
}

.animate-pulse-glow {
  animation: pulse-glow 3s ease-in-out infinite;
}

.animate-gradient {
  background-size: 200% 200%;
  animation: gradient-shift 4s ease infinite;
}

.animate-celebrate {
  animation: celebrate 0.6s ease-out;
}

.animate-attention-pulse {
  animation: attention-pulse 2s ease-in-out infinite;
}

.animate-score-reveal {
  animation: score-reveal 0.5s ease-out;
}

/* Glass card effect */
.glass-card {
  background: linear-gradient(135deg, rgba(30, 27, 75, 0.9) 0%, rgba(88, 28, 135, 0.5) 50%, rgba(30, 27, 75, 0.9) 100%);
  backdrop-filter: blur(20px);
  border: 2px solid rgba(99, 102, 241, 0.3);
  box-shadow:
    0 0 80px -20px rgba(99, 102, 241, 0.3),
    inset 0 1px 0 0 rgba(255, 255, 255, 0.05);
}

/* Neon glow effects */
.glow-orange {
  box-shadow:
    0 0 20px rgba(249, 115, 22, 0.4),
    0 0 40px rgba(249, 115, 22, 0.2),
    0 0 60px rgba(249, 115, 22, 0.1);
}

.glow-pink {
  box-shadow:
    0 0 20px rgba(236, 72, 153, 0.4),
    0 0 40px rgba(236, 72, 153, 0.2);
}

.glow-cyan {
  box-shadow:
    0 0 20px rgba(6, 182, 212, 0.4),
    0 0 40px rgba(6, 182, 212, 0.2);
}

/* Card hover glow effect */
.card-hover-glow {
  transition: box-shadow 0.3s ease;
}

.card-hover-glow:hover {
  box-shadow: 0 0 25px rgba(99, 102, 241, 0.2);
}

/* Success glow for high scores */
.glow-success {
  box-shadow: 0 0 20px rgba(16, 185, 129, 0.3);
}

/* Warning glow for attention needed */
.glow-warning {
  box-shadow: 0 0 20px rgba(239, 68, 68, 0.2);
}

/* Text glow */
.text-glow {
  text-shadow: 0 0 30px rgba(255, 255, 255, 0.3);
}

.text-glow-orange {
  text-shadow: 0 0 30px rgba(249, 115, 22, 0.5);
}

/* Grid pattern overlay */
.bg-grid {
  background-image:
    linear-gradient(rgba(99, 102, 241, 0.5) 1px, transparent 1px),
    linear-gradient(90deg, rgba(99, 102, 241, 0.5) 1px, transparent 1px);
  background-size: 60px 60px;
}

/* Focus indicators */
button:focus-visible, a:focus-visible, input:focus-visible, textarea:focus-visible {
  outline: 2px solid rgba(236, 72, 153, 0.5);
  outline-offset: 2px;
}

/* Readable line length (60-75 characters optimal) */
.prose-readable {
  max-width: 65ch;
}

.prose-readable-sm {
  max-width: 55ch;
}

/* Button Hierarchy System */

/* Primary buttons - gradient, high emphasis, main CTA */
.btn-primary {
  background: linear-gradient(to right, #f97316, #ec4899);
  color: white;
  font-weight: 700;
  padding: 0.75rem 1.5rem;
  border-radius: 0.75rem;
  box-shadow: 0 10px 25px -5px rgba(249, 115, 22, 0.3);
  transition: all 0.2s;
}

.btn-primary:hover {
  opacity: 0.9;
  box-shadow: 0 10px 30px -5px rgba(249, 115, 22, 0.4);
  transform: scale(1.02);
}

.btn-primary:active {
  transform: scale(0.98);
}

/* Secondary buttons - outlined, medium emphasis */
.btn-secondary {
  background: rgba(99, 102, 241, 0.1);
  color: #a5b4fc;
  font-weight: 600;
  padding: 0.625rem 1.25rem;
  border-radius: 0.75rem;
  border: 1px solid rgba(99, 102, 241, 0.3);
  transition: all 0.2s;
}

.btn-secondary:hover {
  background: rgba(99, 102, 241, 0.2);
  border-color: rgba(99, 102, 241, 0.5);
  color: #c7d2fe;
}

/* Tertiary buttons - text-only, low emphasis */
.btn-tertiary {
  color: #818cf8;
  font-weight: 500;
  padding: 0.5rem 1rem;
  border-radius: 0.5rem;
  transition: all 0.2s;
}

.btn-tertiary:hover {
  background: rgba(99, 102, 241, 0.1);
  color: #a5b4fc;
}

/* Ghost buttons - minimal, for dense UIs */
.btn-ghost {
  color: #6366f1;
  font-weight: 500;
  padding: 0.375rem 0.75rem;
  border-radius: 0.5rem;
  transition: all 0.2s;
}

.btn-ghost:hover {
  background: rgba(99, 102, 241, 0.1);
}

/* Hide scrollbar for horizontal scroll */
.scrollbar-hide::-webkit-scrollbar {
  display: none;
}
.scrollbar-hide {
  -ms-overflow-style: none;
  scrollbar-width: none;
}

/* Reduce motion */
@media (prefers-reduced-motion: reduce) {
  *, *::before, *::after {
    animation-duration: 0.01ms !important;
    animation-iteration-count: 1 !important;
    transition-duration: 0.01ms !important;
  }
}
```
