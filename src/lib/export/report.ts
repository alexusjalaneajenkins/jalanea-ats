...
import type { TargetingArtifact } from '../types/targeting';
...
export interface TargetedDraftExportPayload {
...
/**
 * Exports the targeted draft workflow as plain text.
 */
export function exportTargetedDraftToText({
  artifact,
}: TargetedDraftExportPayload): string {
  return artifact.tailoredDraft.draftText;
}

/**
 * Exports the targeted draft workflow as JSON.
 */
export function exportTargetedDraftToJSON(
  payload: TargetedDraftExportPayload,
  ...
  const exportData = {
    meta: {
      ...
      appVersion: APP_VERSION,
      appName: APP_NAME,
      format: 'targeted-json',
    },
    target: {
      title: payload.artifact.parsedJobData.titleHint,
      company: payload.artifact.parsedJobData.companyHint,
      summary: payload.artifact.tailoredDraft.summaryLine,
      readiness: payload.artifact.atsReview.readiness,
    },
...
/**
 * Exports the targeted draft workflow as Markdown.
 */
export function exportTargetedDraftToMarkdown({
  artifact,
}: TargetedDraftExportPayload): string {
  const lines: string[] = [];

  lines.push('# Targeted Resume Draft');
  lines.push('');
  lines.push(`**Generated by:** ${APP_NAME}`);
  lines.push(`**Exported:** ${formatDate(new Date().toISOString())}`);
  lines.push(`**Target role:** ${artifact.parsedJobData.titleHint || 'Untitled role'}`);
  if (artifact.parsedJobData.companyHint) {
    lines.push(`**Company:** ${artifact.parsedJobData.companyHint}`);
  }
  lines.push(`**ATS readiness:** ${artifact.atsReview.readiness}`);
  lines.push('');
  lines.push('---');
  lines.push('');

  lines.push('## Final Draft');
  lines.push('');
  lines.push('```');
  lines.push(artifact.tailoredDraft.draftText);
  lines.push('```');
  lines.push('');

  lines.push('## Focus Areas');
  lines.push('');
  artifact.tailoredDraft.focusAreas.forEach((item) => lines.push(`- ${item}`));
  lines.push('');

  lines.push('## Employer Intent');
  lines.push('');
  lines.push(`- **Primary need:** ${artifact.employerIntent.primaryNeed}`);
  artifact.employerIntent.pressurePoints.forEach((item) => lines.push(`- ${item}`));
  lines.push('');

  lines.push('## Tailoring Suggestions');
  lines.push('');
  artifact.tailoredDraft.suggestions.forEach((suggestion) => {
    lines.push(`### ${capitalizeWord(suggestion.section)} · ${capitalizeWord(suggestion.action)}`);
    lines.push('');
    lines.push(`- **Instruction:** ${suggestion.instruction}`);
    lines.push(`- **Why:** ${suggestion.rationale}`);
    lines.push(`- **Guardrail:** ${suggestion.guardrail}`);
    lines.push('');
  });

  lines.push('## ATS Review');
  lines.push('');
  lines.push(`- **Parse health:** ${artifact.atsReview.parseHealth}%`);
  lines.push(`- **Keyword coverage:** ${artifact.atsReview.keywordCoverage}%`);
  if (artifact.atsReview.recruiterSearch !== undefined) {
    lines.push(`- **Recruiter search:** ${artifact.atsReview.recruiterSearch}%`);
  }
  lines.push(`- **Knockout risk:** ${artifact.atsReview.knockoutRisk}`);
  artifact.atsReview.checklist.forEach((item) => lines.push(`- ${item}`));
  lines.push('');

  if (artifact.iterationHistory.length > 0) {
    lines.push('## Iteration History');
    lines.push('');
    artifact.iterationHistory.forEach((snapshot) => {
      lines.push(`- **${snapshot.label}** (${formatDate(snapshot.createdAt)})`);
      snapshot.changeSummary.slice(0, 3).forEach((item) => lines.push(`  - ${item}`));
    });
    lines.push('');
  }

  lines.push(`*Generated by [${APP_NAME}](https://jalanea.dev)*`);
  lines.push('');

  return lines.join('\n');
}

export function downloadTargetedDraftAsText(payload: TargetedDraftExportPayload): void {
  const content = exportTargetedDraftToText(payload);
  const filename = generateFilename(
    payload.resumeFileName.replace(/\.[^.]+$/, '') + '-targeted',
    'txt'
  );
  downloadFile(content, filename, 'text/plain;charset=utf-8');
}

export function downloadTargetedDraftAsJSON(payload: TargetedDraftExportPayload): void {
  const content = exportTargetedDraftToJSON(payload);
  const filename = generateFilename(
    payload.resumeFileName.replace(/\.[^.]+$/, '') + '-targeted',
    'json'
  );
...
export function downloadTargetedDraftAsMarkdown(payload: TargetedDraftExportPayload): void {
  const content = exportTargetedDraftToMarkdown(payload);
  const filename = generateFilename(
    payload.resumeFileName.replace(/\.[^.]+$/, '') + '-targeted',
    'md'
  );
...
function getKnockoutRiskLabel(risk: 'low' | 'medium' | 'high'): string {
  switch (risk) {
    case 'low':
      return '🟢 Low - You appear to meet the key requirements';
    case 'medium':
      return '🟡 Medium - Some requirements need verification';
    case 'high':
      return '🔴 High - You may not meet critical requirements';
  }
}
